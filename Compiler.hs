...
compileCmd cmd t =
  do appendFile "output.html" canvasCmd
     return t'
  where
    t' = interpretCmd cmd t
    canvasCmd = case cmd of
      Fd x -> case pen t of
        PenUp -> "ctx.moveTo" <> show (pos t') <> ";"
        PenDown -> "ctx.lineTo" <> show (pos t') <> ";"
      _ -> mempty
...
compile :: String -> IO ()
compile s =
  do writeFile "output.html" startTemplate
